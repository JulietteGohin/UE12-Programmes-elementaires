...
# %%
# votre code
tab=np.zeros((91, 91, 3), dtype=np.uint8)
T=tab+255
plt.imshow(T)
plt.show()

T[:,:,:]=0 #sélection de tous le tableau
plt.imshow(T)
plt.show()
...
T[::10, :, :2]=0 #1 ligne sur 10
T[::10, :, 2:]=255 
T[:, ::10, 2:]=255 #1 colonne sur 10
T[:, ::10, :2]=0
plt.imshow(T)
...
np.ndim(im) #3 dimensions

print(np.shape(im)) #533 lignes, 800 colonnes

im.itemsize #la taille en octet d'un élément

im.dtype #le type d'un élément du tableau

np.max(im) #valeurs extremales des pixels
np.min(im)

imextraite=im[:10, :10, :]
plt.imshow(imextraite)
...
def isoler(l, c):
    (n,p,q)=np.shape(im)
    I=im[(n//2)-(l//2):(n//2)+(l-l//2), (p//2)-(c//2):(p//2)+(c-c//2)]
    print(" sdtfgh" , I.shape)
    plt.imshow(I)
...
pixel=im[-1][-1]
np.shape(pixel)
P=pixel.reshape(1,1,3) #reshape pour avoir les bonnes dimensions d'une image
plt.imshow(P)
...
plt.imshow(im[:, :, 0:1], cmap='Reds') #isoler le 1er entier d'un pixel qui correspond au rouge: il faut alors préciser que ca coreespond à du rouge
plt.show()
plt.imshow(im[:, :, 1:2], cmap='Greens')
plt.show()
plt.imshow(im[:, :, 2:3], cmap='Blues')
plt.show()

I=copy.copy(im)
I[-1-201:-2, -1-201:-2, :]=(219, 112, 147)
plt.imshow(I)
plt.show()
# Je ne comprends pas pourquoi la dernière ligne et la dernière colonne ne sont pas prises


# %%
I2=copy.copy(im)
I2[-1-201:-2, -1-201:-2]=(255, 255, 255)
plt.imshow(I2)
I2[-1-201:-2:2, -1-201:-2]=(255, 0, 0)
plt.imshow(I2)
# Je ne comprends pas pourquoi la dernière ligne et la dernière colonne ne sont pas prises


# %%
plt.imshow(I2[-20:-2, -20:-2])
...
im=plt.imread("les-mines.jpg")
(n, p, q)=np.shape(im)
I=np.empty((n,p,q+1), dtype=np.uint8)
I[:, :, :3]=im
I[:, :, 3]=128
plt.imshow(I)
...
# %% {"scrolled": false}
im=plt.imread("les-mines.jpg")
I=im.astype(dtype=np.float64)
I=I/255
plt.imshow(I)
plt.show()

Igris1=im.astype(dtype=np.float64)
Igris1=(I[:, :, 0]+I[:, :, 1]+I[:, :, 2])/3
print('1ere méthode')
plt.imshow(Igris1, cmap='gray')
plt.show()

Igris2=im.astype(dtype=np.float64)
Igris2=0.299*I[:, :, 0]+0.587*I[:, :, 1]+0.114*I[:, :, 2]
print('2eme méthode')
Igris2=Igris2*255   # Pour conversion en Uint8
Imggrise=Igris2.astype(dtype=np.uint8)
plt.imshow(Imggrise, cmap='gray')
plt.show()
...
Iracine=np.power(I, 1/2)
plt.imshow(Iracine, cmap='gray')
plt.show()

Igris8=Iracine*255
Igris8.astype(dtype=np.uint8)
print('3eme méthode')
plt.imshow(Igris8, cmap='gray') #il y a un souci mais je ne sais pas pourquoi...
...
# %%
fig, axes=plt.subplots(1, 3) #définir la forme de la grille

axes[0][0]=plt.imshow(Igris1) #Quelle commande pour attribuer une image à une case ?
axes[0][1]=plt.imshow(Imggrise)
axes[0][2]=plt.imshow(Igris8)




# %%
fig, axes=plt.subplots(2, 3) #définir la forme de la grille mais je ne connais pas la commande pour attribuer une image à une case de la grille...
