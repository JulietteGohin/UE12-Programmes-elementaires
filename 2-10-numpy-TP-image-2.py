...
with open (fichier, 'r') as f:
    couleurs=f.readlines()
print(couleurs)
type(couleurs)

#je pense que le format le plus adapté est un array mais tous les éléments ne sont pas du même type...
...
print(np.shape(im))
tab=np.empty((np.shape(im)[0], np.shape(im)[1], 1))
tab[:, :, 0]=im[:, :, 0]+im[:, :, 1]+im[:, :, 2]
plt.imshow(tab)
plt.show()

print('le maximum est', np.max(tab))
print('l image est de type', type(tab))

tab2=np.empty((np.shape(im)[0], np.shape(im)[1], 1))
tab2[:, :, 0]=np.sum(im, axis=2)
plt.imshow(tab2)
plt.show()

print('le maximum est', np.max(tab2))
print('l image est de type', type(tab2))
...
Imggrise[Imggrise>=127]=255 #avec des filtres booléens, sélections des éléments du tableau vérifiant la condition
Imggrise[Imggrise<127]=0
plt.imshow(Imggrise, cmap='gray')
plt.show()
