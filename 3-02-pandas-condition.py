...
L = ['Survived', 'Pclass', 'Sex', 'Embarked']
for x in L:
    print (pd.Series.unique(df[x]))
...
for x in L:
    print(df[x].dtypes)
#pas besoin de int64, des uint8 suffisent
...
#Nbr de valeurs manquantes par ligne
print(df.isna().sum(axis=1))

#Nbr de valeurs manquantes par colonne
print(df.isna().sum(axis=0))

#Nbr total de valeurs manquantes
print('il manque',(df.isna().sum()).sum(),'valeurs')

Nbtotal = df.shape[0]
print('Nombre total de passagers :', Nbtotal)
...
Nbfemmes = femmes.sum()
F=(Nbfemmes/Nbtotal)*100
M=100-F
print('il y a', F, '% de femmes et', M, '% d hommes')

# %%
adultes = (df['Age']>=20) & (df['Age']<=40)
print('ratio =', adultes.sum()/Nbtotal)

# %%
#1 = mort, 0 = vivant
survivants = df['Survived']==0
print('taux de survie =', survivants.sum()/Nbtotal)

# %%
#Taux de survie des femmes:
for n in range (1,4):
    F = (df['Sex']=='female') & (df['Pclass']==n) &(df['Survived']==1)
    print ('taux de survie des femmes de classe', n, '=', F.sum()/Nbfemmes)
    
#Taux de survie des hommes:
Nbhommes = (df['Sex']=='male').sum()
for n in range (1,4):
    F = (df['Sex']=='male') & (df['Pclass']==n) & (df['Survived']==1)
    print ('taux de survie des hommes de classe', n, '=', F.sum()/Nbhommes)
